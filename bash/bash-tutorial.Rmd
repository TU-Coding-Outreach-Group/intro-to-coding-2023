...
author:
  - name: "**Content creator: Haroon Popal & Liz Beard**"
...
## Why use bash?
Generally, what can be done in one programming language, can be done in any programming language. But somethings are easier to do in particular languages. With Bash, it's easy to do simple things, but very complicated to do complicated things. That may seem obvious, but in some languages like python, doing complicated things can sometimes just be one line of code. In this tutorial, we will go over some of the easier, basic functions of bash. Here are some examples of easy things that can be done in bash:
...
### PC users
I liked to use the [git for windows](https://gitforwindows.org/) bash implementation. Download this programming and install like you would any other program on windows. Then search for the "Git Bash" program. 
...
## Terminal
The terminal is a way of interacting with your computer that does not require a graphic user interface (GUI). In the olden times, computers did not have a mouse, a desktop, folders, or program icons. If you wanted to do something on a computer, you had to give the computer very specific instructions (sometimes referred to as commands). If the terminal is the "old" way of doing things, why do people still use the terminal today?  

One reason, and a point we will stress throughout the bootcamp, is for automation. When you interact with a computer as you regularly do with a GUI and a mouse, you will have to repeat very redundant motions. Let's say you want to rename 100 files. You will have to right click on each one, hit "rename", and then type out the new name of the file, all while making small but infuriating mistakes at each of these steps. Repeating this process over and over, you might also make small mistakes that can be harder to catch later, like if you were to add an extra "a" in a name somewhere. With the terminal, and with coding in general, we can save ourselves a ton of time by creating a set of commands, testing them on a couple of files, and then running them on all of the other files using a script or our text file once we know that they work like we want them to. If later we want to do a similar thing for a new set of files, or even change the names of the files we already edited, we're even closer to completing that goal than we were in the first place because we can just modify that original script. 

***PLEASE NOTE***: We have done this bootcamp annually for a few years now. Some of the screenshots (including that below) were taken when we'd conducted this bootcamp in the past. As such, the year in the title of the bootcamp directory shown may be slightly different than what we'd enter now. For example, if you open your terminal, your version of this directory will likely be named `intro-to-coding-2023` *rather than* `intro-to-coding-2021` as shown in the image below. 
...
# Directories
Another word you might see used is "subdirectory", which is just a directory within a directory. Directories are relative and so any given directory can be a subdirectory, depending on your point of reference. One import directory to know is the **home directory**. Computers have a default directory where you generally start once you open the terminal. If you ever find yourself somewhere and want to go to the home directory, just use the "change directory" command:
...
[Click for solution](https://github.com/TU-Coding-Outreach-Group/intro-to-coding-2023/blob/main/bash/solutions/ex1.sh)
...
To navigate through directories, we can use the `cd` command and input a subdirectory to go to there. For example, use the following command to go to the "Desktop" from the home directory:
...
Here, we only needed one step to get to where we needed to go. For example, we are already on Broad St and the next stop is our destination, City Hall. But what about when you are a few steps away from your destination?
...
#### Exercise 2.a
Navigate to the directory that your COG bootcamp materials are in using the terminal. You will probably have to give multiple directions in order to get to your destination. Navigate to each directory (streets in our City Hall example) and then arrive at your desired directory. Also note that this might be different from person to person depending upon where you downloaded and stored your files to. 
...
[Click for solution](https://github.com/TU-Coding-Outreach-Group/intro-to-coding-2023/blob/main/bash/solutions/ex2a.sh)
...
[Click for solution](https://github.com/TU-Coding-Outreach-Group/intro-to-coding-2023/blob/main/bash/solutions/ex2b.sh)
...
[Click for solution](https://github.com/TU-Coding-Outreach-Group/intro-to-coding-2023/blob/main/bash/solutions/ex2c.sh)
...
[Click for solution](https://github.com/TU-Coding-Outreach-Group/intro-to-coding-2023/blob/main/bash/solutions/ex2d.sh)
...
[Click for solution](https://github.com/TU-Coding-Outreach-Group/intro-to-coding-2023/blob/main/bash/solutions/ex2e.sh)
...
[Click for solution](https://github.com/TU-Coding-Outreach-Group/intro-to-coding-2023/blob/main/bash/solutions/ex2f.sh)
...
[Click for solution](https://github.com/TU-Coding-Outreach-Group/intro-to-coding-2023/blob/main/bash/solutions/ex2g.sh)
...
The logical arguments used here can be a variety of things. One thing you can use it for in data management is to see whether certain files exist. The following command will check to see whether a .csv file exists for a particular subject.
```{eval=FALSE}
# The -e flag tells bash "We want to know if this file exists"
if [ -e subject_data/subj_ID.csv]; then

  # The echo command makes bash print the statement that follows it, but because it's placed in an If-statement, it'll only print it if the logical argument above it is true.
  echo "The data exists!"

fi
```

Oh no! We didn't get any output from that. Maybe let's try looking for the *opposite*; checking if the file *doesn't* exist by adding a bang (`!`) before the flag
...
[Click for solution](https://github.com/TU-Coding-Outreach-Group/intro-to-coding-2023/blob/main/bash/solutions/ex3.sh)
...
[Click for solution](https://github.com/TU-Coding-Outreach-Group/intro-to-coding-2023/blob/main/bash/solutions/ex4.sh)
...
### R
[See instructions on R day page](https://github.com/TU-Coding-Outreach-Group/intro-to-coding-2023//tree/main/R)
