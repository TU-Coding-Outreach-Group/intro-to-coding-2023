...
![python_meme1.png](attachment:python_meme1.png)
...
For those using a Mac: just open up your regular terminal.
For those using a PC: type "cmd" into your computer's search bar.
...
![python_initalize.png](attachment:python_initalize.png)
...
![python_bash.png](attachment:python_bash.png)
...
![anaconda_view.png](attachment:anaconda_view.png)
...
![spyder.png](attachment:spyder.png)
...
![spyder_view.png](attachment:spyder_view.png)
...
*Data types* - much like the real world, data within Python can have different types. Our objects can hold many different dara types. Some data types that you could encounter within Python:
1. Text 
2. Numeric
3. Sequence
4. Mapping
...
A list stores multiple data points within itself. This type of data is particularly common amongst those who do data manipulation within Python. This is an example of a sequence data type.

The most common data types you will encounter in this workshop are text, numeric and sequence. These are all also data types that are common  amongst other coding languages, though not bash or R in this case.
...
Python provies a wide range of possibilities for what we can do with our data. Most of the time, it is pretty intuitive to figure out how to use them. 

To manipulate data we are using something called an oeprator. An operator is a symbol that will carry out a specific function, such as a arithmetic or logical computation. They can be used on variables and values within Python.
...
You can also add what is called an "if statement". Sometimes, you want to run through all of your data, but you only want to retain some of it. The if statment tells the data to do something only if it meets a condition that you have set. For example, if you only wanted to print "psychology" as a major:
...
For loops used in conunction with if statments can be powerful tools in cleaning large datasets. By using something that can iterate through all of your data and extract what you need, it removes human error in data cleaning and makes it a much faster process! Note how we've indented the if statement within the for loop.
...
You can also use if statments within a while loop, just like a for loop. 

### Exercise 4

Create a while loop (with an if statement if you want to get creative) that prints the last half of the reaction times in our variable df!
...
Sometimes when you are coding, you have something that you may want done many times. Rather than code this multiple times in multipe different places, you can create a function which will call a line of code. You can make a function as simple or complex as you would like, and the ability to pass different data into these functions can help shorten your code.
...
Indexing is used to look at individual data points or individual rows and columns based on their location in a sequence. Python is a zero-indexed program, so things like lists start at 0, rather than 1. This means that when Python reads your data in, it will read your first data point in as 0. So, if you were to have 12 data points in your 'Reaction Time' coulmn, then the first data point would be referenced as position 0, and your last data point would be referenced as 11. The easiest way to show this will be when we start working with the data. 
...
We will be using more libraries as we move along, so I will be sure to point these out for you. If you are not sure of what library you might need for a function you would like to perform, [PySearch](pysearch.com) can be an easy tool to use!
Now that we will be working with real data, let's talk about how we can load data in Python.
...
```{python}
path = "C:/Users/tui81100/Documents/GitHub/intro-to-coding-2023/"
```
...
```
Libraries are often used in this format, where it is the *'library'.'function_to_perform'*. You can find a huge list of some of the things you can do in Pandas [here](https://pandas.pydata.org/docs/user_guide/10min.html#object-creation). There are commands to organize your data, select certain rtows or columns, and even perform statistical functions to the data. We have used some of these already, and will continue to in the rest of the workshop!
...
As I just said, this df variable is now our DataFrame. This is exactly what you would imagine it to be - a 2D representation of our tabular data. DataFrames can hold any type of data (text, numeric, sequence, etc). This is helpful because data is not always numeric. 
...
What do you think this '.iloc' is based on what you have learned so far? What library could it be a part of?
Sometimes it is also helpful to print out all of the header names, as it is usually easier to select columns by their name rather than the way we just did it:
...
Now let's see when we filder by gender:
...
Now we can see that our sleep_m variable only contains data where the subjects all of MRI data and have a gender identify of male!
...
Save a CSV file that contains just 10 participants' data, with only those whole all of their MRI data! 
...
Of course, this will not work because the idea behind this is be able to use each item in that list individually. Here is an example with the first 10 subjects of our original DataFrame.
```{python}
...
This should leave you with 10 separate CSV files wherever you deem to place them. Note that we replace the participant number in the string using what's called an f-string - using the letter f to incidcate that whatever is in brackets will be replaced with our variable 'k'. This is generally referred to as string replacement/substiution. We also had to split up the path file to insert our participant number, which is why we "added" our two strings together.
...
Note that pd.append() will be replaced by pd.concat() in the future. This sometimes happens as Python is updated, so be aware of this when you are troubleshooting!

Sometimes, when you are combining DataFrames, you only care about something like the average across all subjects for your data. You can also do that, like this:
...
One common website to find your answers one will be [StackOverflow](https://stackoverflow.com/). This website deals with issues posed about coding. It is a forum, so anyone can post (including you!). Generally, it is often a website that comes up if you Google a quesion that cannot immediately be solved by a library that is already within Python. You can Google and see if this website comes up, or you can also search through their archives. If you cannot find the answer to your question, you can also post the issue that you are encountering and people will reply to let you know how to fix this issue!
...
![python_meme4.jfif](attachment:python_meme4.jfif)
# Final Challenge
...
*Hint*: We haven't taught you how to randomize numbers yet - see if you can use your troubleshooting skills to find an answer online! 
[Click for solution](https://github.com/TU-Coding-Outreach-Group/intro-to-coding-2023/blob/main/python/solutions/py_final.py)
![python_meme3.jpg](attachment:python_meme3.jpg)